...
def fit(feature_extractor, task_classifier, adv_classifier, data, device, model_path, output):
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (x, y, f, f_norm) in enumerate(train_loader):  # iterations loop
            ...
            # forward pass
            h_conv = feature_extractor(x_concat)          
            ...
            task_loss = loss_fn(y_task, y_concat)
            ...
            sys.stdout.write("\r" + '........{}-th mini-batch loss: {:.3f}'.format(i, task_loss.item()))
            sys.stdout.flush()
            
        # Validation
        tr_loss, tr_acc, tr_apcer, tr_bpcer, tr_eer, tr_bpcer_apcer1, tr_bpcer_apcer5, tr_bpcer_apcer10, tr_apcer1, tr_apcer5, tr_apcer10 = eval_model(feature_extractor, task_classifier, adv_classifier, train_loader, device)
        ...
        val_loss, val_acc, val_apcer, val_bpcer, val_eer, val_bpcer_apcer1, val_bpcer_apcer5, val_bpcer_apcer10, val_apcer1, val_apcer5, val_apcer10 = eval_model(feature_extractor, task_classifier, adv_classifier, valid_loader, device)
    ...
    return train_history, valid_loader, best_epoch+1
...
def eval_model(feature_extractor, task_classifier, adv_classifier, data_loader, device, debug=False):
    with torch.no_grad():
        ...
        for iteration in range(NUM_ITERATIONS):
            ...
            for test_material in range(NUM_MATERIALS):
                ...
                if mode == 'train':
                    ...
                    # Fit model
                    train_history, _, best_epoch = fit(feature_extractor=feature_extractor,
                ...
                # Test results
                test_loss, test_acc, test_apcer, test_bpcer, test_eer, test_bpcer_apcer1, test_bpcer_apcer5, test_bpcer_apcer10, test_apcer1, test_apcer5, test_apcer10 = eval_model(feature_extractor, task_classifier, adv_classifier, test_loader, DEVICE)
