...
from data_gen import get_data_loaders
...
def fit(model, adv_weight, transfer_weight, data, n_fake, device, model_path, output, dataset, fake_material):
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (x, y, f, f_norm) in enumerate(train_loader): 
            ...
            if x_fake.shape[0] < 5 or x_real.shape[0] < 5:
              continue
        ...
        # Validation
        tr_loss, tr_acc, tr_apcer, tr_bpcer, tr_eer, tr_bpcer_apcer1, tr_bpcer_apcer5, tr_bpcer_apcer10, tr_apcer1, tr_apcer5, tr_apcer10 = valid_model(model, train_loader, n_fake, adv_weight, transfer_weight, device, dataset, fake_material)
        ...
        val_loss, val_acc, val_apcer, val_bpcer, val_eer, val_bpcer_apcer1, val_bpcer_apcer5, val_bpcer_apcer10, val_apcer1, val_apcer5, val_apcer10 = valid_model(model, valid_loader, n_fake, adv_weight, transfer_weight, device, dataset, fake_material)
...
def valid_model(model, data_loader, n_fake, adv_weight, transfer_weight, device, dataset, fake_material):
        ...
        for iteration in range(NUM_ITERATIONS):
            ...
            for test_material in range(NUM_MATERIALS):
                ...
                if mode == 'train':
                    ...
                    (train_loader, valid_loader, test_loader) = get_data_loaders(IMG_PATH, DATASET, test_material, IMG_SIZE, BATCH_SIZE, croped=True, unseen_attack=unseen)
