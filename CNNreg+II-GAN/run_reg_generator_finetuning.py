...
def fit(model, adv_weight, transfer_weight, data, n_fake, device, model_path, output, dataset, fake_material):
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (x, y, f, f_norm) in enumerate(train_loader): 
            ...
            y_real = y_real.type(torch.LongTensor).to(device)
            fake_label = torch.zeros_like(y_fake).type(torch.LongTensor).to(device)
            label_adv = torch.cat([y_real, fake_label], dim=0)
        ...
        for iteration in range(NUM_ITERATIONS):
            ...
            for test_material in range(NUM_MATERIALS):
                ...
                if mode == 'train':
                    ...
                    (train_loader, valid_loader, test_loader) = get_data_loaders(IMG_PATH, DATASET, test_material, 224, 10, croped=True, unseen_attack=unseen)
