...
def pairwise_loss(signers_combo_list, h_list):
    ...
    # loss *= 1/len(signers_combo_list)
    return loss

def signer_transfer_loss(h_dense_split, signers_on_batch):
    # generate pairwise signer combo list
    signers_combo_list = list(itertools.combinations(signers_on_batch, 2))
    # print(signers_on_batch, signers_combo_list)
    ...
    loss = dense_loss * 1/len(signers_combo_list)
