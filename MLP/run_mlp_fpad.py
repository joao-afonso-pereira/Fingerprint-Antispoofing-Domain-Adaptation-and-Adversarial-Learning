...
from mlp_utils import _nanargmin
...
def getPlots(DETC, num_materials, num):
...
def fit(model, data, device, model_path, output):
    ...
    for epoch in range(EPOCHS):
        ...
        # Validation
        tr_loss, tr_acc, tr_apcer, tr_bpcer, tr_eer, tr_bpcer_apcer1, tr_bpcer_apcer5, tr_bpcer_apcer10, tr_apcer1, tr_apcer5, tr_apcer10, _ = eval_model(model, train_loader, device)
        ...
        val_loss, val_acc, val_apcer, val_bpcer, val_eer, val_bpcer_apcer1, val_bpcer_apcer5, val_bpcer_apcer10, val_apcer1, val_apcer5, val_apcer10, _ = eval_model(model, valid_loader, device)
        ...
        if best_val < val_acc:
            torch.save(model.state_dict(), model_path + 'mlp_fpad.pth')
            best_val = val_acc
            best_epoch = epoch
    ...
    plot_fn = output + 'mlp_fpad_history.png'
    plot_train_history(train_history, plot_fn=plot_fn)

    # return best validation model
    model.load_state_dict(torch.load(model_path + 'mlp_fpad.pth'))
...
def eval_model(model, data_loader, device, debug=False):
    with torch.no_grad():
        ...
        return loss_eval, acc, APCER, BPCER, EER, BPCER_APCER1, BPCER_APCER5, BPCER_APCER10, APCER1, APCER5, APCER10, DETC
...
def main():
    ...
    for test_material in range(NUM_MATERIALS):
        ...
        # Test results
        test_loss, test_acc, test_apcer, test_bpcer, test_eer, test_bpcer_apcer1, test_bpcer_apcer5, test_bpcer_apcer10, test_apcer1, test_apcer5, test_apcer10, DETC = eval_model(model, test_loader, DEVICE)
        ...
        getPlots(DETC, NUM_MATERIALS, test_material)
